/**
 * 准备内置 DeepSeek Harness 运行时（构建期，幂等）
 *
 * 目标：让用户机器**不装 dsh、不装 Node** 也能使用 Harness。
 * 产物（build/harness-runtime/，随 electron-builder extraResources 打包）：
 *   node/node(.exe)                          —— 独立 Node 运行时（dsh 需要 Node ≥22.18）
 *   dsh/node_modules/@deepseek-ai/dsh/...    —— 固定版本的 dsh 依赖树
 *
 * 为什么不用 Electron 自带的 Node：Electron 33 内置 Node 20.18，缺 `node:sqlite`
 * 与 `import.meta.main`，dsh 起不来。
 *
 * 幂等：已存在且版本标记一致时直接跳过；可用 DSH_RUNTIME_FORCE=1 强制重建。
 * 版本可用环境变量覆盖：NODE_RUNTIME_VERSION / DSH_VERSION。
 * 用法：node scripts/prepare-harness-runtime.cjs [--platform win32 --arch x64]
 */
const { spawnSync } = require('child_process')
const fs = require('fs')
const https = require('https')
const os = require('os')
const path = require('path')

const ROOT = path.resolve(__dirname, '..')
const OUT_DIR = path.join(ROOT, 'build', 'harness-runtime')
const NODE_VERSION = process.env.NODE_RUNTIME_VERSION || '24.15.0'
/** dsh 用 Node ≥22.5 的 node:sqlite 与 ≥22.18 的 import.meta.main */
const DSH_VERSION = process.env.DSH_VERSION || '0.1.5-alpha.1'
const MARKER = path.join(OUT_DIR, 'runtime.json')

function arg(name, fallback) {
  const index = process.argv.indexOf(`--${name}`)
  return index >= 0 && process.argv[index + 1] ? process.argv[index + 1] : fallback
}

const PLATFORM = arg('platform', process.platform)
const ARCH = arg('arch', process.arch)

function log(...args) {
  console.log('[harness-runtime]', ...args)
}

/** 下载到文件（跟随重定向），带简单进度输出 */
function download(url, dest, redirects = 0) {
  return new Promise((resolve, reject) => {
    if (redirects > 5) return reject(new Error(`重定向过多：${url}`))
    const file = fs.createWriteStream(dest)
    https.get(url, (res) => {
      if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
        file.close(() => fs.rmSync(dest, { force: true }))
        res.resume()
        return resolve(download(new URL(res.headers.location, url).href, dest, redirects + 1))
      }
      if (res.statusCode !== 200) {
        file.close(() => fs.rmSync(dest, { force: true }))
        res.resume()
        return reject(new Error(`下载失败 HTTP ${res.statusCode}：${url}`))
      }
      const total = Number(res.headers['content-length'] || 0)
      let received = 0
      let lastLog = 0
      res.on('data', (chunk) => {
        received += chunk.length
        const now = Date.now()
        if (total && now - lastLog > 3000) {
          lastLog = now
          log(`  下载中 ${(received / 1048576).toFixed(1)}/${(total / 1048576).toFixed(1)} MB`)
        }
      })
      res.pipe(file)
      file.on('finish', () => file.close(() => resolve(dest)))
      file.on('error', reject)
    }).on('error', reject)
  })
}

async function fetchNodeRuntime() {
  const nodeDir = path.join(OUT_DIR, 'node')
  const exeName = PLATFORM === 'win32' ? 'node.exe' : 'node'
  const target = path.join(nodeDir, exeName)
  if (fs.existsSync(target)) {
    log(`Node 运行时已存在：${target}`)
    return target
  }
  fs.mkdirSync(nodeDir, { recursive: true })

  if (PLATFORM === 'win32') {
    const url = `https://nodejs.org/dist/v${NODE_VERSION}/win-${ARCH}/node.exe`
    log(`下载 Node ${NODE_VERSION}（${url}）`)
    await download(url, target)
  } else {
    const osName = PLATFORM === 'darwin' ? 'darwin' : 'linux'
    const archive = `node-v${NODE_VERSION}-${osName}-${ARCH}.tar.gz`
    const tmp = path.join(os.tmpdir(), archive)
    log(`下载 Node ${NODE_VERSION}（${archive}）`)
    await download(`https://nodejs.org/dist/v${NODE_VERSION}/${archive}`, tmp)
    const extractDir = path.join(os.tmpdir(), `node-extract-${Date.now()}`)
    fs.mkdirSync(extractDir, { recursive: true })
    const inner = `node-v${NODE_VERSION}-${osName}-${ARCH}/bin/node`
    const tar = spawnSync('tar', ['-xzf', tmp, '-C', extractDir, inner], { encoding: 'utf8' })
    if (tar.status !== 0) throw new Error(`解压 Node 运行时失败：${tar.stderr || tar.stdout}`)
    fs.copyFileSync(path.join(extractDir, inner), target)
    fs.chmodSync(target, 0o755)
    fs.rmSync(extractDir, { recursive: true, force: true })
    fs.rmSync(tmp, { force: true })
  }
  log(`Node 运行时就绪：${target}`)
  return target
}

function installDsh() {
  const dshDir = path.join(OUT_DIR, 'dsh')
  const entry = path.join(dshDir, 'node_modules', '@deepseek-ai', 'dsh', 'lib', 'bin.js')
  if (fs.existsSync(entry)) {
    log(`dsh 依赖树已存在：${entry}`)
    return entry
  }
  fs.mkdirSync(dshDir, { recursive: true })
  fs.writeFileSync(path.join(dshDir, 'package.json'), JSON.stringify({ name: 'harness-runtime', private: true }, null, 2))
  log(`安装 @deepseek-ai/dsh@${DSH_VERSION}（约 270MB，首次较慢）`)
  const npmArgs = ['install', '--prefix', dshDir, '--omit=dev', '--no-audit', '--no-fund', '--loglevel=error', `@deepseek-ai/dsh@${DSH_VERSION}`]
  const command = PLATFORM === 'win32' ? (process.env.ComSpec || 'cmd.exe') : 'npm'
  const args = PLATFORM === 'win32' ? ['/d', '/s', '/c', `npm ${npmArgs.join(' ')}`] : npmArgs
  const result = spawnSync(command, args, { cwd: dshDir, stdio: 'inherit', windowsHide: true })
  if (result.status !== 0 || !fs.existsSync(entry)) {
    throw new Error(`安装 @deepseek-ai/dsh@${DSH_VERSION} 失败（exit=${result.status}）`)
  }
  log(`dsh 依赖树就绪：${entry}`)
  return entry
}

async function main() {
  const expected = { nodeVersion: NODE_VERSION, dshVersion: DSH_VERSION, platform: PLATFORM, arch: ARCH }
  if (!process.env.DSH_RUNTIME_FORCE && fs.existsSync(MARKER)) {
    try {
      const current = JSON.parse(fs.readFileSync(MARKER, 'utf8'))
      if (JSON.stringify(current) === JSON.stringify(expected)) {
        log('内置运行时已是目标版本，跳过')
        return
      }
    } catch { /* 标记损坏则重建 */ }
  }
  log(`准备内置运行时：platform=${PLATFORM} arch=${ARCH} node=${NODE_VERSION} dsh=${DSH_VERSION}`)
  await fetchNodeRuntime()
  installDsh()
  fs.writeFileSync(MARKER, JSON.stringify(expected, null, 2))
  const size = (dir) => {
    let total = 0
    const walk = (p) => {
      for (const item of fs.readdirSync(p, { withFileTypes: true })) {
        const full = path.join(p, item.name)
        if (item.isDirectory()) walk(full)
        else if (item.isFile()) total += fs.statSync(full).size
      }
    }
    try { walk(dir) } catch { /* noop */ }
    return (total / 1048576).toFixed(0)
  }
  log(`完成：${OUT_DIR}（约 ${size(OUT_DIR)} MB）`)
}

/** electron-builder beforePack 钩子入口；直接执行时也走同一逻辑 */
module.exports = async function beforePack() {
  await main()
}

if (require.main === module) {
  main().catch((err) => {
    console.error('[harness-runtime] 失败：', err.message)
    process.exit(1)
  })
}
