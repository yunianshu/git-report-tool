...
async function main() {
  ...
  try {
    ...
    // ── 9. Zcode 链接聚合层：symlink/junction 指向的技能必须能被扫描与迁移 ──
    ...
    fs.symlinkSync(claudeDisabled, path.join(zcodeDisabled, 'old-skill'), 'junction') // 目录级链接同样成立
    ...
    assert.strictEqual(zcodeDaily.enabled, true)
    extensionsService.toggleSkill('zcode', 'daily-report', false)
    assert.ok(fs.existsSync(path.join(zcodeDisabled, 'daily-report')), '禁用应移动链接本体')
    assert.ok(fs.existsSync(path.join(claudeSkills, 'daily-report', 'SKILL.md')), '链接源目录必须原样保留')
    extensionsService.toggleSkill('zcode', 'daily-report', true)
    passed += 1
    console.log('  ✓ Zcode 链接聚合层：junction 技能可扫描、可迁移且不破坏源目录')
