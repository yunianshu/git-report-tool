...
跨平台桌面**开发项目管理**工具（Windows / macOS / Linux）。它**不是 Git 管理系统**——Git 提交只是报告的数据来源之一。以本地 Git 提交为依据自动生成项目开发报告，并可一键部署项目，帮助研发团队掌握进度、驱动交付：

- **项目报告**：扫描本机 Git 仓库，一键生成**项目日报 / 周报 / 双周报 / 月报**，内置 AI 报告助手
...
- **多部署目标（多环境）**：一个项目可配置任意多个部署目标（测试 / 生产 / 多台服务器），每个目标独立保存**服务器主机 / SSH 端口 / 用户名 / 认证方式（密码或私钥）/ 远程部署目录 / 健康检查与凭据**；发布、测试连接、版本查询与回滚均作用于当前选中的目标，旧版单服务器配置自动迁移为「默认环境」
