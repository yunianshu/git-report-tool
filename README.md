...
- 选择日期与项目后点「生成报告」：自动收集所选项目当天**本人**的提交（带提交时刻），工时从当天**首条提交开始计时**（迟到不计时）到下班时间（填报今天且未到下班时算到当前时刻），区间内按提交时刻切分归属项目、自动扣午休、0.5 小时向下取整，最后一条提交之后的剩余时间归该项目；**一个项目当天合并为一条工时记录**：工时为该项目全部提交段之和，说明为去类型前缀的编号列表（与活动报告「复制」的简洁内容一致）
