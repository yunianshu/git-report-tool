...
- 选择日期与项目、填写**实际上班时间**（默认取设置页的上班时间，可按天临时调整）后点「生成报告」：自动收集所选项目当天**本人**的提交，工时与 git 提交时刻完全无关——总工时 = 实际上班时间 → **点击生成报告的时刻**（补填历史日期则到下班时间），扣午休后按 0.5 小时整体向下取整；**各项目按当天提交条数占比分配总工时**（合计恒等于总工时）。一个项目当天合并为一条工时记录，说明为去类型前缀的编号列表（与活动报告「复制」的简洁内容一致）
...
- 禅道与汉印的地址 / 账号 / 密码在「设置 → 一键填报」分别配置（汉印支持厦门汉印 / 江西外协），密码均经系统安全存储加密落盘；下班时间与午休区间可按需调整
