...
- 选择日期与项目、填写**实际上班时间**（默认取设置页的上班时间，可按天临时调整）后点「生成报告」：自动收集所选项目当天**本人**的提交，工时与 git 提交时刻完全无关——总工时 = 实际上班时间 → **点击生成报告的时刻**（与填报日期无关，补填历史日期同样按点击时刻；可在工具条手动指定收工时间，早于上班时间按次日跨夜，如 00:30），扣午休后按 0.5 小时整体向下取整；**各项目按当天提交条数占比分配总工时**（合计恒等于总工时）。一个项目当天合并为一条工时记录，说明为去类型前缀的编号列表（与活动报告「复制」的简洁内容一致）
...
- 汉印按工时占比填报（当日全部条目合计 100%，四舍五入余数自动补偿）；汉印「软件项目」任务与禅道任务同源，绑定禅道任务后自动匹配汉印任务，未匹配到的任务只写禅道并提示
