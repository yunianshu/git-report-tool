...
- 选择日期与项目后点「生成报告」：自动收集所选项目当天**本人**的提交，工时与提交时刻无关——总工时从当天**首条提交**（实际上班的近似，迟到不计时）到**点击生成报告的时刻**（补填历史日期则到下班时间），扣午休后按 0.5 小时整体向下取整；**各项目按当天提交条数占比分配总工时**（合计恒等于总工时）。一个项目当天合并为一条工时记录，说明为去类型前缀的编号列表（与活动报告「复制」的简洁内容一致）
