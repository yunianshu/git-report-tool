...
- **先生成再勾选**：选好日期与上下班时间后直接点「生成报告」，自动收集**当天所有项目**里你本人的提交并逐项列出（无需先选项目）；随后在明细中勾选要填报的项目（默认全勾选，含未绑定的项目以免工时漏报），**勾选变化会立即按新的项目子集重算工时与汉印占比**（复用已采集的提交，不重复跑 git），未勾选的项目不计入填报
- 填写**实际上班时间**（默认取设置页的上班时间，可按天临时调整）：工时与 git 提交时刻完全无关——总工时 = 实际上班时间 → **点击生成报告的时刻**（与填报日期无关，补填历史日期同样按点击时刻；可在工具条手动指定收工时间，早于上班时间按次日跨夜，如 00:30），扣午休后按 0.5 小时整体向下取整；**各项目按当天提交条数占比分配总工时**（合计恒等于总工时）。一个项目当天合并为一条工时记录，说明为去类型前缀的编号列表（与活动报告「复制」的简洁内容一致）
...
- 不配置汉印则只填报禅道，两个平台互不阻断（接口失败时容错提示）
